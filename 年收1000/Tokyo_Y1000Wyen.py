...
async def get_title(i):
    ...
    url = 'https://job.yahoo.co.jp/jobs/?&l=%E6%9D%B1%E4%BA%AC%E9%83%BD&su=YAR&smn=10000000&page={0}&ssid=1af9ffa2-6297-435b-9924-f853eded234c'.format(i)
    ...
    try:
        cursor.executemany('insert into Tokyo_Y1000Wyen (salary,type,link,job_name) values (%s,%s,%s,%s)', big_list)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    except TypeError :
