...
if __name__ == '__main__':
    ...
    count_sql = "select count(*) from Tokyo_Kernel; "
    cur.execute(count_sql)
    long_count = cur.fetchone()['count(*)']
    # sql 语句
    big_list = []
    for num in range(1, long_count):
        sql = 'select salary,type,link,job_name  from Tokyo_Kernel where id = %s ' % num
        # #执行sql语句
        cur.execute(sql)
        # #获取所有记录列表
        data = cur.fetchone()
...
# salary,type,link,job_name
    ...
    csv_dict_write('{0}/Tokyo_Kernel.csv'.format(l_path), head, big_list)
    print("数据导出完成～")