...
async def get_title(i):
    url = 'https://job.yahoo.co.jp/jobs/?&keyword=Bloomberg&l=%E6%9D%B1%E4%BA%AC%E9%83%BD&page={0}&ssid=de1a7414-0dd3-4ea9-a879-2e1f6d267306&comp=1'.format(i)
    ...
    try:
        cursor.executemany('insert into Tokyo_Bloomberg (salary,type,link,job_name) values (%s,%s,%s,%s)', big_list)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    except TypeError :
