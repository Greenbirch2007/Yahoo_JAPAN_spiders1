...
async def get_title(i):
    ...
    url = 'https://job.yahoo.co.jp/jobs/?&keyword=gRPC&l=東京都&page={0}&ssid=25158ae9-0705-4dd3-85a3-30a1930eb123&comp=1'.format(i)
    ...
    try:
        cursor.executemany('insert into Tokyo_gRPC (salary,type,link,job_name) values (%s,%s,%s,%s)', big_list)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    except TypeError :
